...
function Material (props) {
  ...
  return <View style={{ display: 'flex', flexDirection: 'column', marginBottom: '6pt' }}>
    <Text style={{ fontWeight: 'bold', borderBottom: '1px solid black' }}>{ material.instanceName || props.$tc('contentNode.material.name') }</Text>
    { items.map(item => {
      return <View key={item.id} style={{ display: 'flex', flexDirection: 'row' }}>
