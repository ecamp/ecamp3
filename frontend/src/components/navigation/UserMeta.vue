  ...
  <v-menu v-model="open"
    ...
    <v-list dense class="user-nav"
      ...
      <v-list-item tag="li" block
        ...
        <span>{{ $tc('components.Campnavigation.userMeta.profile') }}</span>
      </v-list-item>
      <v-list-item block tag="li"
                   exact :to="{ name: 'camps' }"
                   @click="open = false">
        <v-icon left>mdi-format-list-bulleted-triangle</v-icon>
        <span>{{ $tc('components.Campnavigation.userMeta.myCamps', api.get().camps().items.length) }}</span>
      </v-list-item>
      <v-list-item block tag="li"
                   @click="logout">
        ...
        <v-progress-circular v-else indeterminate
                             size="18" class="mr-2" />
        <span>{{ $tc('components.Campnavigation.userMeta.logOut') }}</span>
      </v-list-item>
    </v-list>
  </v-menu>
</template>
