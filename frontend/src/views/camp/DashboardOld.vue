...
<template>
  <content-card :title="camp().title" toolbar>
    <v-card-text>
      {{ $tc('views.camp.dashboardOld.viewDescription', 1, { title: camp().title }) }}
    </v-card-text>
    ...
    <template v-else>
      <v-card-text>
        <p style="text-align: center">
          {{ $tc('views.camp.dashboardOld.noActivitiesLine1') }} <br />
          {{ $tc('views.camp.dashboardOld.noActivitiesLine2') }} <br />
        </p>
        <p style="text-align: center">
          <v-btn color="success" :to="campRoute(camp(), 'program')">
            <v-icon size="150%" left> mdi-view-dashboard-variant </v-icon>
            {{ $tc('views.camp.dashboardOld.program') }}
          </v-btn>
        </p>
      </v-card-text>
    </template>
