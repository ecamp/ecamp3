<template>
  <v-app-bar app clipped-left color="blue-grey-darken-4" dark>
    ...
    <v-toolbar-items>
      <v-btn :to="campRoute(camp())" variant="text">
        <v-icon :start="$vuetify.display.mdAndUp">mdi-tent</v-icon>
        <span class="sr-only-sm-and-down">{{ camp().title }}</span>
      </v-btn>
      <v-btn :to="campRoute(camp(), 'program')" variant="text">
        <v-icon :start="$vuetify.display.mdAndUp">mdi-view-dashboard</v-icon>
        <span class="sr-only-sm-and-down">{{
          $tc('views.camp.navigation.desktop.navTopbar.program')
        }}</span>
      </v-btn>
      <v-btn :to="campRoute(camp(), 'story')" variant="text">
        <v-icon :start="$vuetify.display.mdAndUp"> mdi-book-open-variant </v-icon>
        <span class="sr-only-sm-and-down">{{
          $tc('views.camp.navigation.desktop.navTopbar.story')
        }}</span>
      </v-btn>
      <v-btn :to="materialListRoute(camp())" variant="text">
        <v-icon :start="$vuetify.display.mdAndUp"> mdi-package-variant </v-icon>
        <span class="sr-only-sm-and-down">{{
          $tc('views.camp.navigation.desktop.navTopbar.material')
        }}</span>
      </v-btn>
      <v-btn :to="campRoute(camp(), 'admin')" variant="text">
